...
clear_image = PhotoImage(file='clear.png').subsample(20)
...
def calculate_derivative(p1, p2):
    ...
    dx = p2[0] - p1[0]
    koeff = dx**2 + dy**22
    return [dx/koeff, dy/koeff]

def draw_hermite():
    global last_curve_pixels
    global pixels
    if len(pixels) == 4:
        p1, p2, r1, r2 = pixels
        t = 0
        x1, x2, x3, x4 = p1[0], p2[0], r1[0]-p1[0], r2[0]-p2[0]
        y1, y2, y3, y4 = p1[1], p2[1], r1[1]-p1[1], r2[1]-p2[1]
        canvas.create_line(x1, y1, x1 + x3, y1 + y3, fill='green', arrow=LAST,
                  arrowshape=(10, 20, 10))
        canvas.create_line(x2, y2, x2 + x4, y2 + y4, fill='green', arrow=LAST,
                  arrowshape=(10, 20, 10))
...
def draw_bezie():
    ...
    if len(pixels) == 4:
        p1, p2, r1, r2 = pixels
        t = 0
        x1, x2, x3, x4 = p1[0], p2[0], r1[0], r2[0]
        y1, y2, y3, y4 = p1[1], p2[1], r1[1], r2[1]
        while t <= 1:
...
def draw_bspline():
    ...
    for i in range(len(pixels)):
        x1, x2, x3, x4 = pixels[0][0], pixels[1][0], pixels[2][0], pixels[3][0]
        y1, y2, y3, y4 = pixels[0][1], pixels[1][1], pixels[2][1], pixels[3][1]
        t = 0
        ...
        while t <= 1:
            ...
            canvas.create_rectangle(x, y, x, y)
            t += 0.001
        pixels = cycle_shift(pixels)
    last_curve_pixels = pixels
    pixels = []
    
def change():
    global pixels
    pixels = last_curve_pixels
    change_window = Tk()
    change_window.geometry("280x300")
    change_window.title('move one point')
    show_ccor = Label(change_window, text='Coordinates: ')
    coord_text = ''
    for k in range(len(pixels)):
        coord_text += f'p{str(k+1)}: ({pixels[k][0]},{pixels[k][1]})\n'
    show_ccor['text'] = f'Coordinates: \n{coord_text}'
    show_ccor.pack()
    var1 = Label(change_window, text='Coordinates to change:')
    var1.pack()
    entr1 = Entry(change_window)
    entr1.pack()
    var2 = Label(change_window, text='input new x and y')
    var2.pack()
    entr21 = Entry(change_window)
    entr21.pack()
    entr22 = Entry(change_window)
    entr22.pack()
    def ent():
        ...
        if not entr21.get() == None and not entr22.get() == None:
            ...
            y = int(y)
            pixels[i-1] = (x, y)
            if pressed_button == hermite:
                draw_hermite()
            elif pressed_button == bezier:
                draw_bezie()
            elif pressed_button == bspline:
                draw_bspline()
    enter = Button(change_window, text='change', command=ent)
    enter.pack()
        
def draw(click):
    global pixels
    if pressed_button in pixeled: x, y = convert_coordinates(click.x, click.y)
    else: x, y = click.x, click.y
    pixels.append((x, y))
    if pressed_button == diff_analyzer:
        ...
        canvas.create_rectangle(x-1, y-1, x+1, y+1, fill=color, outline=color)
        draw_hermite()
    elif pressed_button == bezier:
        canvas.create_rectangle(x-1, y-1, x+1, y+1, fill=color, outline=color)
        draw_bezie()
        ...
        canvas.create_rectangle(x-1, y-1, x+1, y+1, fill=color, outline=color)
    elif pressed_button == bspline_submit:
        draw_bspline()
    else: pixels = []
...
pixeled = [diff_analyzer, brezenhem, smooth, circumference, ellipse, hyperbole, parable]
...
hermite.grid(row=0, column=0, columnspan=2)
bezier = Button(approximation_panel, text='Bezier', pady=3, command=lambda:change_button(bezier))
bezier.grid(row=1, column=0, columnspan=2)
bspline = Button(approximation_panel, text='B-Spline', pady=3, command=lambda:change_button(bspline))
bspline.grid(row=2, column=0)
bspline_submit = Button(approximation_panel, text='B-Spline', pady=3, command=draw_bspline)
bspline_submit.grid(row=2, column=1)
change_curve = Button(approximation_panel, text='redact', pady=3, command=change)
change_curve.grid(row=3, column=0, columnspan=2)
...
clear = Button(toolbar, image=clear_image, pady=3, command=clear_canvas)
